...
revision = "0016_uc03_default_documents"
...
def upgrade() -> None:
    # Existing empty published versions caused UC03 to snapshot an empty checklist.
    # Retire only truly empty versions; non-empty Admin configuration is untouched.
    op.execute(
        """
        UPDATE auditcore.document_requirement_profile_versions v
        SET lifecycle_status='RETIRED',
            retired_by_actor_id='migration.0016.uc03-default-documents',
            retired_at_utc=now(),
            updated_at_utc=now()
    ...
    # Publishing a header without document rows is never a valid Project master.
    op.execute(
        """
    ...
    # One function is used both for existing Projects and for future provisioning.
    # It creates a normal versioned Project master, not a Web hard-coded checklist.
    op.execute(
        r"""
        ...
        BEGIN
            ...
            FROM (VALUES
                -- Booking: Booking initiates the Journey. These are the default
                -- evidence requirements from the approved business process.
    ...
    # Existing Projects receive the default now. CURRENT_DATE deliberately makes
    # this repaired usable version win over older empty/legacy profiles.
    op.execute(
        """
    ...
    # Future Projects get the same default during provisioning. Admin may later
    # publish a new version; existing Journeys retain their snapshot/version.
    op.execute(
        """
    ...
    # Repair only Journeys that were created against a profile with no document
    ...
    # The original stage triggers run only on stage creation, so explicitly
    # snapshot requirements for already-created broken Booking/Delivery stages.
    op.execute(
        """
...
def downgrade() -> None:
    # Keep historical/snapshotted requirements intact. Remove only the automatic
    # provisioning/validation hooks; the migration is intentionally data-safe.
    op.execute(
