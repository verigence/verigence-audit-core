...
Repository/CI, PostgreSQL foundation, Security/error/request-context, Project landscape/assignment, and versioned-master increment E are complete. F-01 Customer APIs and F-02 protected project-wide customer matching are complete. F-03 Journey APIs are VERIFIED: Journey routes are registered, configured Journey status codes are respected, hierarchy/master-version/audit-separation tests pass, and GitHub Actions run `31891744663` passed build, lint, fresh migration and all tests. The G-01 cross-module authentication architecture decision is resolved by `VAC-SD-AUTH-001`. Default onboarding RBAC is approved by `VAC-SD-RBAC-001` with canonical Audit Core catalogue `design/AUDIT_CORE_SECURITY_CATALOG_v2.1.json`: PC, TL, PM and CRM are cross-module default templates containing the required `audit.*` and `di.*` permissions. Security dependencies `SEC-INT-001` and `SEC-RBAC-001` are implemented and verified, including persistent Tenant role overrides controlled by Tenant Admin/Super Admin. G-01 remains IN PROGRESS pending the controlled Audit Core -> Security -> DI integration verification required by its acceptance gate.
...
| F | Customer and Journey | 3 | 2 | VERIFIED |
| G | Internal DI façade | 5 | 0 | IN PROGRESS |
...
| F-03 | Implement Journey APIs | VERIFIED | `src/audit_core/journeys.py` implementation plus route registration commit `b85a82114792eb1678c6bded55b576f9172cc231`; configured Journey-status test fixture corrected in `ae9526e22b66fff796f6607e2bfec099a55b4e43`; `tests/test_journeys.py` verifies hierarchy, exact published master references, observed-status/audit separation and rejection of audit-state mutation through normal Journey PATCH; GitHub Actions run `31891744663` passed build, lint, fresh PostgreSQL migration and all tests | Acceptance verified; ready for COMPLETE transition |
| G-01 | Verify Audit Core→DI authentication mechanism | IN PROGRESS | Architecture decision approved in `docs/AUDIT_CORE_CROSS_MODULE_AUTH_DESIGN_v1.0.md`; default cross-module roles approved in `docs/AUDIT_CORE_DEFAULT_ROLE_BUNDLES_v1.0.md` and `design/AUDIT_CORE_SECURITY_CATALOG_v2.1.json`; Security `SEC-INT-001` and `SEC-RBAC-001` are COMPLETE; Security CI runs `31884091488` and `31884922331` passed OAuth/JWKS, role-boundary, admin-override and PostgreSQL persistence tests | Security dependencies resolved. Remaining G-01 acceptance evidence is the controlled Audit Core -> Security -> DI call proving delegated and service paths without direct client DI access or private bypass |
