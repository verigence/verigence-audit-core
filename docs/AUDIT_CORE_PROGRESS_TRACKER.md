...
Repository/CI, PostgreSQL foundation, Security/error/request-context, Project landscape/assignment, and versioned-master increment E are complete. F-01 Customer APIs and F-02 protected project-wide customer matching are complete. F-03 Journey API code/tests are committed but its latest CI run failed, so F-03 remains IN PROGRESS. The G-01 cross-module authentication architecture decision is resolved by `VAC-SD-AUTH-001`, including the rule that Security user tokens carry the Tenant's effective cross-module platform permission bundle while downstream token exchange narrows authority to the required DI capability. G-01 implementation remains BLOCKED until Security implements `SEC-INT-001` for platform permission resolution, service-token issuance and OAuth delegated token exchange.
...
| G-01 | Verify Audit Core→DI authentication mechanism | BLOCKED | Architecture decision approved in `docs/AUDIT_CORE_CROSS_MODULE_AUTH_DESIGN_v1.0.md`; platform-permission-bundle clarification committed in `83cda048cad971ff129966a0a44e3148a548773e`; Security implementation task is `SEC-INT-001` in `verigence-security` | Architecture decision RESOLVED. Implementation waits for Security to issue user tokens with effective cross-module platform `permissions[]`, Tenant-scoped `SERVICE` tokens, and OAuth delegated token exchange/on-behalf-of tokens narrowed to requested downstream authority; no private bypass or service-token fallback for new user actions |
