...
Repository/CI, PostgreSQL foundation, Security/error/request-context, Project landscape/assignment, and versioned-master increment E are complete. F-01 Customer APIs and F-02 protected project-wide customer matching are complete. F-03 Journey API code/tests are committed but its latest CI run failed, so F-03 remains IN PROGRESS. The G-01 cross-module authentication architecture decision is resolved by `VAC-SD-AUTH-001`. Security dependency `SEC-INT-001` is now implemented and verified: Security issues effective cross-module platform permissions, Tenant-scoped `SERVICE` tokens and narrowed OAuth delegated tokens. G-01 is therefore IN PROGRESS pending the controlled Audit Core -> Security -> DI integration verification required by its acceptance gate.
...
| G-01 | Verify Audit Core→DI authentication mechanism | IN PROGRESS | Architecture decision approved in `docs/AUDIT_CORE_CROSS_MODULE_AUTH_DESIGN_v1.0.md`; platform-permission-bundle clarification commit `83cda048cad971ff129966a0a44e3148a548773e`; Security `SEC-INT-001` marked COMPLETE in `verigence-security` commit `ba423aa9730c559a8fa9bacc5a04c509d7f13c5b`; Security CI run `31884091488` passed Ruff and OAuth/JWKS authorization tests | Security dependency resolved. Remaining G-01 acceptance evidence is the controlled Audit Core -> Security -> DI call proving delegated and service paths without direct client DI access or private bypass |
