...
- `UC03_EXTRACTION_CORRECTION_POLICY_2026-08-25.md` requirements that Audit Core persist/use DI confidence or invoke TL behavior for a PC extraction correction.
...
5. **DI owns immutable machine evidence:** original document, extraction, source facts, fact versions, page/box localization, processing state, and any confidence calculated internally.
6. **Audit Core never persists DI confidence.** Confidence is not part of the DI->Audit linkage, Booking upload status, or PC-approved business data.
...
- internal confidence where DI already calculates it.
...
The client orchestrates the user experience only. It does not become a source of truth for document processing, linkage, or approved business values.
...
4. Confidence is not required by this PC UI contract.
...
- persist minimum provenance (`documentId`, `sourceFactRef`, `sourceFactVersion`, decision, actor, timestamp);
- append the appropriate extraction-approved/extraction-corrected audit/workflow record;
- commit atomically.

Audit Core does **not** persist:
...
- confidence-based Audit Core flags;
- confidence display in PC UI;
...
7. Audit Core stores no DI confidence and no copied machine extraction merely for display.
8. Screen 3 obtains document status/content/extraction/page/box directly from DI.
9. PC can approve/correct supported extracted fields.
10. Audit Core commits the approved/corrected business values plus DI provenance and audit history atomically in one batch transaction per reviewed document.
11. Existing R2 hierarchy is unchanged.
12. No TL or Delivery behavior is introduced.
