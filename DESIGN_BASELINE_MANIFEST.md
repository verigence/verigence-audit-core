...
**Approved role-bundle amendment:** `VAC-SD-RBAC-001` / Audit Core Default Operational Role Bundles v1.0 — **APPROVED ARCHITECTURE DECISION**  
**Approved UC03 customer-mobile PII amendment:** `docs/uc-003-booking-delivery-audit/UC03_CUSTOMER_MOBILE_PII_DESIGN_2026-08-27.md` — **IMPLEMENTATION APPROVED**  
**Current Security permission catalogue:** `design/AUDIT_CORE_SECURITY_CATALOG_v2.2.json` — **APPROVED FOR SECURITY REGISTRATION**  
...
| `docs/AUDIT_CORE_DEFAULT_ROLE_BUNDLES_v1.0.md` | `VAC-SD-RBAC-001` | APPROVED ARCHITECTURE DECISION / DEFAULT ROLE BUNDLES |
| `docs/uc-003-booking-delivery-audit/UC03_CUSTOMER_MOBILE_PII_DESIGN_2026-08-27.md` | UC03 customer mobile PII amendment | IMPLEMENTATION APPROVED |
| `design/AUDIT_CORE_SECURITY_CATALOG_v2.2.json` | Audit Core permission catalogue v2.2 | APPROVED FOR SECURITY REGISTRATION |
...
22. **Default operational RBAC is cross-module and Tenant-customizable:** Security provides PC, TL, PM and CRM onboarding templates containing approved `audit.*` and `di.*` permissions. Tenant Admin may customize those four templates for its Tenant; Super Admin may customize Tenant templates and platform defaults. The resolved Security authorization decision remains authoritative, PC has no default formal verification-write authority, and destructive/platform-admin DI permissions are excluded from these operational templates.
23. **Complete customer mobile is stored in Audit Core, not masked in persistence.** Existing Customer APIs mask the value unless Security authorizes `audit.customer.contact.full.read`; the permission is granted by default only to Executive while SuperAdmin retains the existing full-authority rule for ACTIVE registered permissions.
...
- `design/AUDIT_CORE_SECURITY_CATALOG_v1.0.json` — REGISTRATION HOLD / SUPERSEDED BY v2.2
...
The project owner approved the current v2.1 package for implementation on 2026-08-15. `VAC-SD-003`, `VAC-API-001`, `VAC-ERR-001`, `VAC-DM-002`, `VAC-DB-002` and `api/openapi-v1.yaml` are therefore the formal Audit Core implementation baseline. `VAC-SD-AUTH-001` is an approved architecture amendment to that baseline for cross-module authentication and authorization. `VAC-SD-RBAC-001`, the UC03 customer-mobile PII amendment, and `design/AUDIT_CORE_SECURITY_CATALOG_v2.2.json` are the approved role/permission amendments for Security registration and Tenant onboarding. Material changes must follow requirements/design/API/schema change control rather than being introduced silently during implementation.
...
The v1.0 Audit Core Security catalogue remains on hold. Security SHALL register/use the v2.2 Audit Core permission catalogue and the approved operational role templates/amendments. Security must continue to implement the service-token and delegated OAuth token-exchange capabilities defined by `VAC-SD-AUTH-001`; no direct DI authentication bypass is permitted.
